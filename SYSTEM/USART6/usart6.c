...
int UsartRcvIMUYesenseHandle(u8 res){
	if((USART6_RX_STA&(1<<15))==0)//接收完的一批数据,还没有被处理,则不再接收其他数据
	{ 
		if(USART6_RX_STA<USART6_MAX_SEND_LEN)		//还可以接收数据
		{
			if(res==0x59&&USART6_RX_STA==0)
			{
				USART6_RX_BUF[USART6_RX_STA++]=res;		//第一个头字符
			}
			else if(res==0x49&&USART6_RX_STA==1)
			{
				USART6_RX_BUF[USART6_RX_STA++]=res;		///第二个头字符
			}
			else if(res==0x53&&USART6_RX_STA==2)
			{
				USART6_RX_BUF[USART6_RX_STA++]=res;		///第三个头字符
			}
			else if(USART6_RX_STA>2)
			{
				USART6_RX_BUF[USART6_RX_STA++]=res;		
			}
			
			
			if(USART6_RX_STA>126)
			{
				USART6_RX_STA|=1<<15;	//接收完成
