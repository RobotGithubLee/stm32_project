...
u16 USART2_RX_STA=0;   	 
int UsartRcvRobotDataHandle2(u8 Res);
...
{  
  ...
  USART_ITConfig(USART2, USART_IT_RXNE, ENABLE);//开启中断   
	...
	//TIM5_Int_Init(100-1,8400-1);		//10ms中断
	USART2_RX_STA=0;		//清零
	//TIM_Cmd(TIM5, DISABLE); //关闭定时器6
...
{  
	...
	for(i=0;i<len;i++)//循环发送数据
	{
	  while(USART_GetFlagStatus(USART2,USART_FLAG_TC)==RESET);  //等待上次传输完成 
		USART_SendData(USART2,buf[i]); 	 //发送数据到串口2 
	}
}
...
int UsartRcvIMUDataHandle2(u8 res){
	if((USART2_RX_STA&(1<<15))==0)//接收完的一批数据,还没有被处理,则不再接收其他数据
	{ 
		if(USART2_RX_STA<USART2_MAX_SEND_LEN)		//还可以接收数据
		{
